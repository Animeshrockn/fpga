...
//------------------------Parameter----------------------
`define TV_IN_gmem "../tv/cdatafile/c.kernel_gemm.autotvin_gmem.dat"
`define TV_OUT_gmem "../tv/rtldatafile/rtl.kernel_gemm.autotvout_gmem.dat"
...
output [32 - 1 : 0] TRAN_gmem_C_V;
output [32 - 1 : 0] TRAN_gmem_A_V;
output [32 - 1 : 0] TRAN_gmem_B_V;
input ready;
input done;
...
assign    TRAN_gmem_C_V = C_V;
assign    TRAN_gmem_A_V = A_V;
assign    TRAN_gmem_B_V = B_V;
...
initial begin : initialize_offset
  ...
  C_V <= 0 * DATA_byte_num;
  A_V <= 256 * DATA_byte_num;
  B_V <= 512 * DATA_byte_num;
end
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("/localhdd/kss24/kernel_gemm_4_DRAM_reorg/kernel_gemm_buffering.prj/solution1/sim/verilog/AESL_axi_master_gmem.v: Write request address %d exceed AXI master gmem array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("/localhdd/kss24/kernel_gemm_4_DRAM_reorg/kernel_gemm_buffering.prj/solution1/sim/verilog/AESL_axi_master_gmem.v: Read request address %d exceed AXI master gmem array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
